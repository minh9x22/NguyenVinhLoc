...
app = Flask(__name__)
app.config['SECRET_KEY'] = 'BaraVN Python-Flask Web App'
...
def main():
    todolist = [
        {
            'name': 'Buy milk',
            'desciption': 'Buy 2 liters of milk in Coopmart.'
        },
        {
            'name': 'Get money',
            'desciption': 'Get 500k from ATM'
        },
    ]
    return render_template('index.html', todolistvar = todolist)
...
def signUp():
    ...
    if form.validate_on_submit():
        ...
        if(db.session.query(models.User).filter_by(email=_email).count() == 0):
            ...
            db.session.add(user)
            db.session.commit()
            return render_template('signUpSuccess.html', userpass = user)
        else:
            flash('Email {} is already exits'.format(_email))
...
def userHome():
    ...
    if _user_id:
        ...
        tasks = db.session.query(models.Task).filter_by(user_id=_user_id, isCompleted=False).all()
        return render_template('userhome.html', user = user, tasks=tasks)
    else:
        return redirect('/')
